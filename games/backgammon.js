...
exports.parseBackgammon = parseBackgammon;
exports.serializeBackgammon = serializeBackgammon;
exports.backgammonGetValidMoves = backgammonGetValidMoves;
exports.backgammonRollDice = backgammonRollDice;
exports.backgammonApplyMove = backgammonApplyMove;
...
const NUM_POINTS = 24;
function parseBackgammon(text) {
    if (!text.startsWith("GAME::BACKGAMMON::"))
        return null;
...
function serializeBackgammon(s) {
...
function backgammonGetValidMoves(state, playerNum, dieValue) {
    const color = playerColor(playerNum);
...
function backgammonRollDice(state, playerNum) {
    if (state.winner !== 0)
        return null;
    if (state.turn !== playerNum)
        return null;
    if (state.remainingMoves.length > 0)
        return null; // Already rolled
    const d1 = crypto_1.default.randomInt(1, 7);
    const d2 = crypto_1.default.randomInt(1, 7);
    const dice = [d1, d2];
    // Doubles = 4 moves
...
function backgammonApplyMove(state, from, to, playerNum) {
    if (state.winner !== 0)
        return null;
    if (state.turn !== playerNum)
        return null;
    ...
    if (usedIdx !== -1)
        newRemaining.splice(usedIdx, 1);
    // Check win
    let winner = 0;
    if (newOffWhite === 15)
        winner = 1;
    else if (newOffBlack === 15)
        winner = 2;
...
//# sourceMappingURL=backgammon.js.map