...
export function parseXO(text: string): XOState | null {
    if (!text.startsWith("GAME::XO::")) return null;
...
export function serializeXO(s: XOState): string {
...
export function xoMakeMove(state: XOState, pos: number, playerNum: number): XOState | null {
    if (pos < 0 || pos > 8) return null;
    if (state.winner !== 0) return null;
    if (state.turn !== playerNum) return null;
    if (state.board[pos] !== 0) return null;
