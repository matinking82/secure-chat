...
export interface ChaarBargState {
    ...
    p2: string;
    turn: number;        // 1 or 2
    phase: number;       // 0=waiting,1=playing,2=roundOver,3=matchOver
    p1Score: number;     // cumulative points
    p2Score: number;
    hands: [Card[], Card[]];
    center: Card[];
    captures: [Card[], Card[]]; // captured piles
    drawPile: Card[];
    lastCapture: number; // 1 or 2 (who captured last)
    p1Surs: number;
    p2Surs: number;
    roundStarter: number; // 1 or 2 (who started this round)
}
...
export function parseChaarBarg(text: string): ChaarBargState | null {
    if (!text.startsWith("GAME::CHAARBARG::")) return null;
...
export function serializeChaarBarg(s: ChaarBargState): string {
...
export function getCaptureCombinations(playedCard: Card, center: Card[]): Card[][] {
    const results: Card[][] = [];
...
export function chaarBargPlayCard(
    state: ChaarBargState,
    playerNum: number,
    cardIndex: number,
    captureChoice: number // index into getCaptureCombinations result, -1 = no capture (place on table)
): ChaarBargState | null {
    ...
    const newCaptures: [Card[], Card[]] = [[...state.captures[0]], [...state.captures[1]]];
...
function calculateRoundScores(state: ChaarBargState): ChaarBargState {
    let p1Points = 0;
    let p2Points = 0;
    ...
    for (let pIdx = 0; pIdx < 2; pIdx++) {
        ...
        for (const card of pile) {
            ...
            if (card.rank === 2 && card.suit === 4) points += 2;
            // 10 of Diamonds (suit 3) = 3 points
            if (card.rank === 10 && card.suit === 3) points += 3;
        }

        // Sur points (5 each)
        if (pIdx === 0) points += state.p1Surs * 5;
        else points += state.p2Surs * 5;

        if (pIdx === 0) p1Points = points;
        else p2Points = points;
    }
    ...
    if (p1Clubs > p2Clubs) p1Points += 7;
    else if (p2Clubs > p1Clubs) p2Points += 7;
    // tie = no bonus
...
/** Start a new round */
export function chaarBargStartRound(state: ChaarBargState): ChaarBargState {
...
export function chaarBargNewRound(state: ChaarBargState): ChaarBargState | null {
